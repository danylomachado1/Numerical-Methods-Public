...
def A_1(N,L,delta_t):
    ...
    delta_t = delta_t
    delta_x = L/(N-1)
    lamb = (D*delta_t)/(delta_x)**2
    #lamb = 1
    A_1 = zeros((N-2,N-2), dtype = float)
    for i in range(N-2):
        if i == 0:
            A_1[i,i] = 2*(1+lamb)
            A_1[i,i+1] = -1*lamb
...
def A_2(N,L,delta_t):
    ...
    lamb = (D*delta_t)/(delta_x)**2
    #lamb = 1
    A_2 = zeros((N-2,N-2), dtype = float)
    for i in range(N-2):
        if i == 0:
            A_2[i,i] = 2*(1+C*delta_t-lamb)
            A_2[i,i+1] = 1*lamb
...
def Crank_Nicolson(N,L,t_final,delta_t):
    t = 0
    #print(t)
    Val = []
    ...
    while t <= t_final:
        ...
        C = GaussSeidel(A_1(N,L,delta_t),dot(A_2(N,L,delta_t),C))
        if t in linspace(0,5,11)[1:]:
            #print(t)
            #print(C)
            Val.append(C)
...
def ajuste_error_final(t,n_hat,lab1, lab2, mark):
    #lab1 = "Error obtenido: metodo"
    #lab2 = "Linearizacion-metodo"
    X_t = copy(t)
    Y_t = copy(n_hat)
    ...
    text3 = '$r$ = ' + str(round(sqrt(r2),5))
    text4 = '$A , α$ = ' + str(round(A,4))+','+str(round(alfa,4))

    lab2 = lab2 + "\nA, α = "+ str(round(A,4))+','+str(round(alfa,4))+"\nr ="+ str(round(sqrt(r2),5))
    ...
    X_linearizado = linspace(t[0],t[-1],300)
    f_x_linearizado = []
    for x_i in X_linearizado:
        f_x_linearizado.append(A*exp(x_i/alfa))

    if mark == "True":
        fig, axes = plt.subplots()
        axes.plot(t,n_hat,"ko",markersize = 4, label = lab1)
        axes.plot(X_linearizado,f_x_linearizado , "r-",markersize = 1, label = lab2  )
    ...
    return X_linearizado,f_x_linearizado
